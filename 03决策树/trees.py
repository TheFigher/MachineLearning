...
def chooseBestFeatureToSplit(dataSet):
    ...
    for i in range(numFeatures):  # 循环遍历数据集中的所有特征
        featList = [example[i] for example in dataSet]  # 使用列表推导来创建新的列表 将数据集中所有第i个特征值或者所有可能存在的值写入这个新list中
        uniqueVals = set(featList) # 转换成集合数据类型
        newEntropy = 0.0
...
def majorityCnt(classList):
    ...
    return sortedClassCount[0][0]  # 并返回出现次数最多的分类名称
...
def creatTree(dataSet, labels):  # 数据集和标签列表(包含了数据集中所有特征的标签)
    ...
    if classList.count(classList[0]) == len(classList):  # 所有的类标签完全相同，则直接返回该类标签
        return classList[0]
    if len(dataSet[0]) == 1: # 使用完了所有特征，仍然不能将数据集划分成仅包含唯一类别的分组（挑选出现次数最多的类别作为返回值）
